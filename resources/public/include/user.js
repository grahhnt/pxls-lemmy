  ...
  const self = {
          ...
          crel(
            ...
            crel(
              ...
              Object.values(data.authServices).map((service) => {
                const anchor = crel(
                  "a",
                  { href: `/signin/${service.id}?redirect=1` },
                  service.name
                );
                ...
                if (!service.registrationEnabled) {
                  crel(
                    ...
                    crel(
                      ...
                      {
                        style:
                          "font-style: italic; font-size: .75em; font-weight: bold; color: red; margin-left: .5em",
                      },
                      __("New Accounts Disabled")
                    )
                  );
                }
                return toRet;
    ...
    init: function (instaban) {
              ...
              crel(
                ...
                crel(
                  "button",
                  {
                    ...
                    onclick: () =>
                      self.doSignOut().then(() => modal.closeAll()),
                  },
                  __("Yes")
                ),
                crel(
      ...
      socket.on("userinfo", function (data) {
        ...
        place.togglePaletteSpecialColors(
          data.placementOverrides.canPlaceAnyColor
        );
        ...
        if (data.banExpiry === 0) {
          ...
          crel(
            ...
            crel(
              "p",
              __(
                `You are temporarily banned and will not be allowed to place until ${timestamp}`
              )
            )
          );
        ...
        if (isBanned) {
          self.elements.userMessage
            ...
            .show()
            .text(
              __("You can contact us using one of the links in the info menu.")
            )
            .fadeIn(200);
          crel(
            banelem,
            crel(
              "p",
              __(
                "If you think this was an error, please contact us using one of the links in the info tab."
              )
            ),
            crel("p", __("Ban reason:")),
            crel("p", data.banReason)
          );
          modal.show(modal.buildDom(crel("h2", __("Banned")), banelem), {
            escapeClose: false,
            clickClose: false,
          });
          if (window.deInitAdmin) {
    ...
    _handleRenameSubmit: function (event) {
      ...
      if (!input || !input.value || !btn || !err)
        return console.error(
          "Missing one or more variables from querySelector. input: %o, btn: %o, err: %o",
          input,
          btn,
          err
        );
      ...
      $.post("/execNameChange", { newName: input.value.trim() }, function () {
        ...
        .fail(function (xhrObj) {
          let resp = __(
            "An unknown error occurred. Please contact staff on discord"
          );
    ...
    _handleRenameClick: function (event) {
      const renamePopup = crel(
        ...
        crel(
          "p",
          __(
            "Staff have required you to change your username, this usually means your name breaks one of our rules."
          )
        ),
        crel(
          "p",
          __(
            "If you disagree, please contact us on Discord (link in the info panel)."
          )
        ),
        crel(
          "label",
        ...
        crel(
          ...
          {
            style:
              "display: none; font-weight: bold; color: #f00; font-size: .9rem;",
            class: "rename-error",
          },
          ""
        ),
        crel(
          ...
          crel(
            "button",
            { class: "rename-submit text-button", type: "submit" },
            __("Change")
          )
        )
      );
    ...
    updatePixelCountElements: () => {
      self.elements.pixelCounts
        .find("#current-pixel-count")
        .text(self.pixelCount.toLocaleString());
      self.elements.pixelCounts
        .find("#alltime-pixel-count")
        .text(self.pixelCountAllTime.toLocaleString());
    },
  };
  return {
    init: self.init,
