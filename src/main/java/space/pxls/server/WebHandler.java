...
public class WebHandler {
	...
	public WebHandler() {
		addServiceIfAvailable("lemmy", new LemmyAuthService("lemmy"));
	...
	public void view40x(HttpServerExchange exchange, int x, User user) {
		...
		String toRet = String.format(
				"<p style=\"text-align: center;\">Socc broke the %d page! Let someone know please :) <a href=\"/\">Back to Root</a></p>",
				x);
		try {
	...
	public void profileView(HttpServerExchange exchange) {
		...
		if (user == null) {
			...
			String requested = match.getParameters().compute("who",
					(k, s) -> (s == null || s.isEmpty()) ? user.getName() : s);
			boolean requested_self = user.getName().equals(requested); // usernames are case-sensitive, we can use
																		// #equals relatively safely.
			...
			if (profileUser == null) {
				...
				try {
					...
					List<Faction> factions = App.getDatabase().getFactionsForUID(profileUser.getId()).stream()
							.map(Faction::new).collect(Collectors.toList());
					...
					m.put("palette", App.getPalette().getColors().stream().map(color -> color.getValue())
					...
					if (requested_self) {
						...
						m.put("chat_reports_open_count",
								chatReports.stream().filter(dbChatReport -> !dbChatReport.closed).count());
						m.put("canvas_reports", canvasReports);
						m.put("canvas_reports_open_count",
								canvasReports.stream().filter(dbCanvasReport -> !dbCanvasReport.closed).count());
					}
								...
								if (faction.getOwner() == user.getId()) {
									...
									if (user.getDisplayedFaction() != null && user.getDisplayedFaction() == fid) {
										user.setDisplayedFaction(null, false, true); // displayed_faction is updated by
																						// #leaveFaction() already. we
																						// just need to invalidate the
																						// memcache.
									}
								}
							}
							...
							if (dataObj.has("owner")) {
								...
								if (_owner != null) {
									...
									User toSet = faction.fetchMembers().stream()
											.filter(n -> n.getName().equals(final_owner)).findFirst().orElse(null);
									if (toSet != null) {
	...
	private void setAuthCookie(HttpServerExchange exchange, String loginToken, int days) {
		...
		exchange.setResponseCookie(new CookieImpl("pxls-token", "").setPath("/").setExpires(pastCalendar.getTime()));

		Calendar futureCalendar = Calendar.getInstance();
		futureCalendar.add(Calendar.DATE, days);
		String hostname = App.getConfig().getString("host");
		exchange.setResponseCookie(new CookieImpl("pxls-token", loginToken).setHttpOnly(true)
				.setSameSiteMode((exchange.isSecure() ? CookieSameSiteMode.NONE : CookieSameSiteMode.LAX).toString())
				.setSecure(exchange.isSecure()).setPath("/").setDomain("." + hostname)
				.setExpires(futureCalendar.getTime()));
		exchange.setResponseCookie(new CookieImpl("pxls-token", loginToken).setHttpOnly(true)
				.setSameSiteMode((exchange.isSecure() ? CookieSameSiteMode.NONE : CookieSameSiteMode.LAX).toString())
				.setSecure(exchange.isSecure()).setPath("/").setDomain(hostname).setExpires(futureCalendar.getTime()));
	}
	...
	public void chatban(HttpServerExchange exchange) {
		...
		boolean isPerma = false, isUnban = false;
	...
	public void discordNameChange(HttpServerExchange exchange) {
		exchange.getResponseHeaders().put(Headers.CONTENT_TYPE, "application/json");
		sendBadRequest(exchange, "Not permitted");
//		User user = exchange.getAttachment(AuthReader.USER);
//		if (user == null) {
//			sendBadRequest(exchange, "No authenticated user could be found");
//			return;
...
//				sendBadRequest(exchange,
//						"name isn't in the discord tag format (only english letters, numbers, periods and underlines allowed)");
//				return;
//			}
//		}
//
	...
	public void check(HttpServerExchange exchange) {
		...
		if (data != null) {
			...
			if (user != null) {
				...
				exchange.getResponseSender()
						.send(App.getGson()
								.toJson(new ExtendedUserInfo(user.getName(), user.getAllRoles(), user.getLogins(),
										user.getPixelCount(), user.getAllTimePixelCount(), user.isBanned(),
										user.getBanExpiryTime(), user.getBanReason(),
										user.loginsWithIP() ? "ip" : "service", user.getPlaceOverrides(),
										!user.canChat(), App.getDatabase().getChatBanReason(user.getId()),
										user.isPermaChatbanned(), user.getChatbanExpiryTime(),
										user.isRenameRequested(true), user.getDiscordName(), user.getChatNameColor())));
			} else {
				exchange.setStatusCode(400);
			}
		}
...
//        String name = LemmyAuthService.getUsername(tokenVal.getValue());
...
//            return;
		...
		User user = App.getUserManager().signUp(token, ip);
...
//        }
	...
	public void auth(HttpServerExchange exchange) throws UnirestException {
		...
		if (service != null && service.use()) {
			...
			exchange.setResponseCookie(
					new CookieImpl("pxls-auth-redirect", "").setPath("/").setExpires(pastCalendar.getTime()));
			...
			if (identifier != null) {
				...
				if (user == null) {
					if (service.isRegistrationEnabled()) {
						...
						if (redirect) {
							redirect(exchange, String.format("/auth_done.html?token=%s&signup=true",
					...
					setAuthCookie(exchange, loginToken, 24);
					if (redirect) {
						redirect(exchange, String.format("/auth_done.html?token=%s&signup=false",
								encodedURIComponent(loginToken)));
					} else {
	...
	public void signIn(HttpServerExchange exchange) {
		...
		if (service != null) {
			...
			if (redirect) {
				exchange.setResponseCookie(new CookieImpl("pxls-auth-redirect", "1")
						.setSameSiteMode(
								(exchange.isSecure() ? CookieSameSiteMode.NONE : CookieSameSiteMode.LAX).toString())
						.setSecure(exchange.isSecure()).setPath("/"));
	...
	public void info(HttpServerExchange exchange) {
		...
		exchange.getResponseHeaders().add(HttpString.tryFromString("Content-Type"), "application/json")
				.add(HttpString.tryFromString("Access-Control-Allow-Origin"), "*");
		exchange.getResponseSender().send(App.getGson().toJson(new CanvasInfo(App.getCanvasCode(), App.getWidth(),
				App.getHeight(), App.getPalette().getColors(),
				...
				(int) App.getConfig().getInt("stacking.maxStacked"), services, App.getRegistrationEnabled(),
				App.isChatEnabled(), Math.min(App.getConfig().getInt("chat.characterLimit"), 2048),
				App.getConfig().getBoolean("chat.canvasBanRespected"), App.getConfig().getStringList("chat.bannerText"),
				App.getSnipMode(), App.getConfig().getList("chat.customEmoji").unwrapped(),
				App.getConfig().getString("cors.proxyBase"), App.getConfig().getString("cors.proxyParam"),
	...
	private String encodedURIComponent(String toEncode) {
		...
		String result = "";
		try {
			result = URLEncoder.encode(toEncode, "UTF-8").replaceAll("\\+", "%20").replaceAll("\\%21", "!")
