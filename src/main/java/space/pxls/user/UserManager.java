...
public class UserManager {
	...
	private User getByDB(Optional<DBUser> optionalUser) {
		...
		return userCache.computeIfAbsent(user.id,
				(k) -> new User(user.id, user.stacked, user.username, user.signup_time, user.cooldownExpiry, roles,
	...
	public User signUp(String token, String ip) {
		UserLogin login = userSignupTokens.get(token);
		if (login == null)
			return null;

//        if (!App.getDatabase().getUserByName(name).isPresent()) {
		DBUser user = App.getDatabase().createUser(login.getServiceUserID(), login, ip);
